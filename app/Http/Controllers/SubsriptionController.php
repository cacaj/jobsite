...
class SubsriptionController extends Controller
{
    ...
    public function __construct(){
        ...
        $this->middleware('hasMembership')->except('subscribe');
    }
...
//                dd($successURl);
    ...
    public function paymentSuccess(Request $request)
    {
        ...
        try {
            Mail::to(auth()->user())->queue(new SubscribeMail($plan, $billingEnds));
        } catch (\Exception $e) {
            return $e;
        }

        return redirect()->route('dashboard')->with('success', 'Payment was succesful.');


    }
    public function cancel(){
        return redirect()->route('dashboard')->with('error','Payment was unsuccesful.');
